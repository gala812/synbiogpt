<script lang="ts">
	...
	const THINKING_STATUS_TEXT = '模型正在思考中...';
	...
	const setThinkingStatus = (responseMessage) => {
		const wasThinking = responseMessage.thinkingStatus === 'thinking';
		responseMessage.statusHistory = [
			{
				action: 'thinking',
				description: THINKING_STATUS_TEXT,
				done: false
			}
		];
		responseMessage.thinkingStatus = 'thinking';
		if (!wasThinking || !responseMessage.thinkingStartAt) {
			responseMessage.thinkingStartAt = Date.now();
			responseMessage.thinkingEndAt = null;
			responseMessage.thinkingDurationSec = null;
		}
	};

	const finishThinking = (responseMessage) => {
		if (responseMessage?.thinkingStatus !== 'thinking') {
			return;
		}

		const endAt = Date.now();
		responseMessage.thinkingStatus = 'done';
		responseMessage.thinkingEndAt = endAt;
		responseMessage.thinkingDurationSec = Math.max(
			1,
			Math.round((endAt - (responseMessage.thinkingStartAt ?? endAt)) / 1000)
		);
	};
	...
	const sendPromptOllama = async (model, userPrompt, responseMessageId, _chatId) => {
		...
		try {
			[res, controller] = await generateChatCompletion(localStorage.token, {
				stream: stream,
				model: model.id,
			...
			finishThinking(responseMessage);
			responseMessage.statusHistory = [];
