...
from open_webui.config import DEFAULT_GROUP_PERMISSIONS
...
def _build_default_group_name(user_name: str, email: str) -> str:
    """
    生成稳定且较短的默认组名，例如：888组-ab12
    后缀基于 email 的 md5 前4位，避免重名。
    """
    suffix = hashlib.md5(email.lower().encode("utf-8")).hexdigest()[:5]
    return f"{user_name}组_{suffix}"


def _create_default_group_for_user(user) -> Optional[dict]:
    """
    为新用户创建默认权限组，并把该用户加入组中。
    如果用户已经属于某个组，则不重复创建。
    """
    existing_groups = Groups.get_groups_by_member_id(user.id)
    if existing_groups and len(existing_groups) > 0:
        return existing_groups[0]

    group_name = _build_default_group_name(user.name, user.email)
    group_description = f"{user.name}的默认权限组"
...
def _ensure_default_group_for_user(user) -> None:
    """
    包一层兜底，避免组创建失败直接影响注册/登录主流程。
    这里只记日志，不抛异常。
    """
    try:
        group = _create_default_group_for_user(user)
        if not group:
            log.warning(f"Failed to create default group for user: {user.id}")
    except Exception as e:
        log.exception(f"Failed to ensure default group for user {user.id}: {e}")
...
async def update_profile(
    ...
    if session_user:
        ...
        user = Users.update_user_by_id(session_user.id, update_data)
        if user:
            return user
        else:
        ...
        if username == form_data.user.lower():
            ...
            if not user:
                try:
                    ...
                    _ensure_default_group_for_user(user)
...
async def signup(request: Request, response: Response, form_data: SignupForm):
    ...
    try:
        ...
        print("signup form_data:", form_data.model_dump())
