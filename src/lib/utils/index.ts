...
export const replaceTokens = (content, sourceIds, char, user) => {
	...
	if (Array.isArray(sourceIds)) {
		const sourceIdSet = new Set(sourceIds.filter((id) => typeof id === 'string'));
		...
		// Handle grouped citations like [a, b] / [a, b with full-width comma] where each item is a known source id.
		content = content.replace(/\[([^\]]+)\]/g, (match, inner) => {
			...
			if (!parts.every((part) => sourceIdSet.has(part))) return match;

			return parts.map((part) => `<source_id data="${part}" />`).join('');
		});
	}
