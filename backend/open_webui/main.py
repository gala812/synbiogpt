...
log = logging.getLogger("synbiogpt.env")
...
class ChatCompletionMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
                ...
                if "document" in source:
                    for doc_idx, doc_context in enumerate(source["document"]):
                        ...
                        if isinstance(metadata, list):
                            doc_metadata = metadata[doc_idx] if doc_idx < len(metadata) else {}
                            if isinstance(doc_metadata, dict):
                                doc_source_id = doc_metadata.get("source", source_id)
                            else:
                                doc_metadata = {}
                        ...
                        normalized_source_id = (
                            ...
                            else (doc_source_id if doc_source_id is not None else source_id)
                        )

                        if normalized_source_id:
                            context_string += f"<source><source_id>{normalized_source_id}</source_id><source_context>{doc_context}</source_context></source>\n"
                        else:
        ...
        if len(citation_sources) > 0:
            log.info(
                "[CITATION] sending sources count=%d first_indices=%s",
        ...
        if not isinstance(response, StreamingResponse):
            log.info(
                "[PERF] rag.llm duration=%.3fs streaming=false model=%s path=%s",
        ...
        async def stream_wrapper(original_generator, data_items):
            ...
            try:
                ...
                log.info(
                    "[PERF] rag.llm duration=%.3fs streaming=true chunks=%d model=%s path=%s",
                    time.perf_counter() - llm_start,
                    forwarded_chunks,
                    body.get("model"),
                    request.url.path,
        ...
        for header in ("content-length", "content-encoding", "transfer-encoding"):
            stream_headers.pop(header, None)
