...
async def update_user_by_id(
    ...
    if user:
        ...
        incoming_info = form_data.info or {}
        ...
        updated_user = Users.update_user_by_id(
            user_id,
            {
                ...
                "info": {**current_info, **incoming_info},
            },
        )
...
async def mark_first_login_done(user=Depends(get_verified_user)):
    Users.update_user_by_id(
        user.id,
        {
            "oauth_sub": f"onboarding_done:{user.id}"
        }
    )
    return {"success": True}