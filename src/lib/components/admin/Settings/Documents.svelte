<script lang="ts">
	...
	const submitHandler = async () => {
		...
		const res = await updateRAGConfig(localStorage.token, {
			...
			chunk: {
				text_splitter: textSplitter === 'character' ? 'character' : '',
	...
	onMount(async () => {
      ...
      const raw = (res?.chunk?.text_splitter ?? '').toString().trim().toLowerCase();
			textSplitter = raw === 'character' ? 'character' : '';
		...
		<div class=" ">
			...
			<div class="flex w-full justify-between mb-1.5">
				...
				<div class="flex items-center relative">
					...
					>
						<option value="">{$i18n.t('Default')} ({$i18n.t('Token')})</option>
