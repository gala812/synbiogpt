<script lang="ts">
	...
	const loadSidebarMeta = async () => {
		const [allTags, pinned] = await Promise.all([
			getAllTags(localStorage.token).catch((error) => {
				toast.error(error);
				return [];
			}),
			getPinnedChatList(localStorage.token).catch((error) => {
				toast.error(error);
				return [];
			}),
			initFolders()
	...
	const initChatList = async (options: { includeMeta?: boolean } = {}) => {
		const { includeMeta = true } = options;
		const metaPromise = includeMeta ? loadSidebarMeta() : Promise.resolve();
		...
		let chatList = [];
		if (search) {
			chatList = await getChatListBySearchText(localStorage.token, search, $currentChatPage).catch(
				(error) => {
					toast.error(error);
					return [];
				}
			...
			chatList = await getChatList(localStorage.token, $currentChatPage).catch((error) => {
				toast.error(error);
				return [];
			});
		}
	...
	onMount(async () => {
		...
		await initChatList({ includeMeta: false });
		setTimeout(() => {
