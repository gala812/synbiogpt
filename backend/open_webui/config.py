...
frontend_splash = FRONTEND_BUILD_DIR / "static" / "default.png"

if frontend_splash.exists():
    try:
        shutil.copyfile(frontend_splash, STATIC_DIR / "default.png")
    except Exception as e:
        logging.error(f"An error occurred: {e}")
else:
    ...
    try:
        ...
        if r.ok:
            ...
            if "splash" in data:
                ...
                r = requests.get(url, stream=True)
                if r.status_code == 200:
                    with open(f"{STATIC_DIR}/default.png", "wb") as f:
                        r.raw.decode_content = True
                        shutil.copyfileobj(r.raw, f)
...
QDRANT_BATCH_SIZE = int(os.environ.get("QDRANT_BATCH_SIZE", "1000"))
QDRANT_SCROLL_LIMIT = int(os.environ.get("QDRANT_SCROLL_LIMIT", "10000"))
