<script>
    ...
    .map((u) => ({
      ...u,
      pdf_upload_limit: u.info?.pdf_upload_limit ?? 20,
      pdf_upload_used: u.info?.pdf_upload_used ?? 0
    }))
		.filter((user) => {
			if (search === '') {
				return true;
...
<div class="scrollbar-hidden relative whitespace-nowrap overflow-x-auto max-w-full rounded pt-0.5">
	...
	>
        ...
        <th
          scope="col"
          class="px-3 py-1.5 cursor-pointer select-none"
          on:click={() => setSortKey('organization')}
        >
          <div class="flex gap-1.5 items-center">
            {$i18n.t('Organization')}
        ...
        <th
          scope="col"
          class="px-3 py-1.5 cursor-pointer select-none"
          on:click={() => setSortKey('pdf_upload_limit')}
        >
          <div class="flex gap-1.5 items-center">
            最大PDF上传数

            {#if sortKey === 'pdf_upload_limit'}
              <span class="font-normal">
                {#if sortOrder === 'asc'}
                  <ChevronUp className="size-2" />
                {:else}
        ...
        <th
          scope="col"
          class="px-3 py-1.5 cursor-pointer select-none"
          on:click={() => setSortKey('pdf_upload_used')}
        >
          <div class="flex gap-1.5 items-center">
            已用PDF上传数

            {#if sortKey === 'pdf_upload_used'}
              <span class="font-normal">
                {#if sortOrder === 'asc'}
                  <ChevronUp className="size-2" />
                {:else}
		...
		<tbody class="">
          ...
          <td class="px-3 py-1">{user.organization ?? ''}</td>
          <td class="px-3 py-1">{user.contact ?? ''}</td>
          ...
          <td class="px-3 py-1">
            {user.info?.pdf_upload_limit ?? 20}
          </td>
          <td class="px-3 py-1">
            {user.info?.pdf_upload_used ?? 0}
          </td>
